...
class ManageUsersController extends Controller {
    ...
    public function detail($id) {
        ...
        $widget['total_withdrawn']   = Withdrawal::where('user_id', $user->id)->where('status', Status::PAYMENT_SUCCESS)->sum('amount');
    ...
    public function sendNotificationAll(Request $request) {
        ...
        foreach ($users as $user) {
            ...
            $subject = $request->subject;
            $viewName = 'email.notify';
    
            try {
                Mail::to($user->email)->send(new NotifyEmail($details, $subject, $viewName));
            } catch (\Exception $e) {
                Log::error('Failed to send email: ' . $e->getMessage(), ['exception' => $e]);
                throw ValidationException::withMessages(['error' => 'Failed to send notification email.']);
            }
        }
