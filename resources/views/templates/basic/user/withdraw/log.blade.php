    ...
    <div class="dashboard-inner">
        <div class="mb-4">
            <div class="d-flex justify-content-between">
                ...
                <span>
                    <a href="{{ route('user.withdraw') }}" class="btn btn--base btn--smd"><i class="las la-long-arrow-alt-left"></i> @lang('Withdraw Money')</a>
                </span>
            </div>
        </div>
        <div class="row">
                            ...
                            <h2 class="accordion-header" id="h-{{ $loop->iteration }}">
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#c-{{ $loop->iteration }}" aria-expanded="false" aria-controls="c-1">
                                    <div class="col-lg-4 col-sm-5 col-8 order-1 icon-wrapper">
                                        <div class="left">
                                            @if ($withdraw->status == Status::WITHDRAW_SUCCESS)
                                                <div class="icon icon-success">
                                                    <i class="las la-check"></i>
                                                </div>
                                            @elseif($withdraw->status == Status::WITHDRAW_PENDING)
                                                <div class="icon icon-warning">
                                                    <i class="las la-spinner fa-spin"></i>
                                                </div>
                                            @elseif($withdraw->status == Status::WITHDRAW_CANCEL)
                                ...
                                <div class="accordion-body">
                                    <ul class="caption-list">
                                        <li>
                                            ...
                                            <span class="value">{{ showAmount($withdraw->charge) }}
                                                {{ $general->cur_text }}</span>
                                        </li>
                                        <li>
                                            <span class="caption">@lang('After Charge')</span>
                                            <span class="value">{{ showAmount($withdraw->amount - $withdraw->charge) }}
                                                {{ __($general->cur_text) }}</span>
                                        </li>
                                        <li>
                                            <span class="caption">@lang('Conversion')</span>
                                            <span class="value">{{ $general->cur_sym }}{{ showAmount($withdraw->amount - $withdraw->charge) }}
                                                x {{ $general->cur_sym }}{{ showAmount($withdraw->rate) }} =
                                                {{ $general->cur_sym }}{{ showAmount($withdraw->final_amount) }}</span>
                                        </li>
                                        <li>
                                            <span class="caption">@lang('Status')</span>
                                            <span class="value">
                                                @php echo $withdraw->statusBadge @endphp <button class="btn text--danger" type="button"><i class="las la-info-circle detailBtn" data-user_data="{{ json_encode($withdraw->withdraw_information) }}" @if ($withdraw->status == 3) data-admin_feedback="{{ $withdraw->admin_feedback }}" @endif></i></button>
                                            </span>
                                        </li>
                                        <li>
                                            <span class="caption">@lang('Phone Number')</span>
                                            <span class="value">{{_($withdraw->phone_number) }}</span>
                                        </li>
                                    </ul>
                                </div>
                            </div>
