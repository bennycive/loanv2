    ...
    <div class="section container">
        <div class="row g-lg-0">
            <div class="col-lg-6 d-none d-lg-block">
                @if ($loginContent && isset($loginContent->data_values->image))
            ...
            <div class="col-lg-6">
                <div class="auth-form__content section">
                    {{-- <h3 class="text-capitalize text-center mt-0 mb-4">
                        {{ $loginContent && isset($loginContent->data_values->heading) ? $loginContent->data_values->heading : '' }}
                    </h3> --}}
                     <h3 class="text-capitalize text-center mt-0 mb-4" >@lang('Login')</h3>
                    <form method="POST" action="{{ route('user.login') }}" class="row g-4 verify-gcaptcha">
                        ...
                        @endphp
                        @if ($credentials && 
                            (isset($credentials->google->status) && $credentials->google->status == Status::ENABLE || 
                             isset($credentials->facebook->status) && $credentials->facebook->status == Status::ENABLE || 
                             isset($credentials->linkedin->status) && $credentials->linkedin->status == Status::ENABLE))
                            <div class="col-12">
                            ...
                            <!-- Rest of your social media login buttons -->
                        @endif
                    </form>
                </div>
            </div>
