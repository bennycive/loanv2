        ...
        <div class="col-12">
            <div class="row gy-4">
                <div class="col-xxl-3 col-xl-8 col-sm-6">
                    <x-widget-2 color="primary" icon="la la-money-bill-wave-alt" title="Balance"
                ...
                <div class="col-xxl-3 col-xl-4 col-sm-6">
                    <x-widget-2 color="success" icon="la la-wallet" title="Deposited"
                        value="{{ $general->cur_sym }}{{ showAmount($widget['total_deposit']) }}"
                        link="{{ route('admin.deposit.list') }}?search={{ $user->username }}" overlay_icon=0
                        icon_style=solid />
                </div>
                <div class="col-xxl-3 col-xl-4 col-sm-6">
                    <x-widget-2 color="danger" icon="la la-wallet" title="Withdrawn"
                        value="{{ $general->cur_sym }}{{ showAmount($widget['total_withdrawn']) }}"
                        link="{{ route('admin.withdraw.log') }}?search={{ $user->username }}" overlay_icon=0
                        icon_style=solid />
                </div>

                <div class="col-xxl-3 col-xl-4 col-sm-6">
                    <x-widget-2 color="info" icon="las la-exchange-alt" title="Transactions"
                        value="{{ $widget['total_transaction'] }}"
                        link="{{ route('admin.report.transaction') }}?search={{ $user->username }}" overlay_icon=0
                        icon_style=solid />
                </div>
                <div class="col-xxl-3 col-xl-4 col-sm-6">
                    <x-widget-2 color="primary" icon="las la-spinner fa-spin" title="Running Loan"
                        value="{{ $widget['running_loan'] }}"
                        link="{{ route('admin.loan.index') }}?status={{ Status::LOAN_RUNNING }}&search={{ $user->username }}"
                        overlay_icon=0 icon_style=solid />
                </div>
                <div class="col-xxl-3 col-xl-4 col-sm-6">
                    <x-widget-2 color="dark" icon="las la-sync" title="Pending Loan"
                        value="{{ $widget['pending_loan'] }}"
                        link="{{ route('admin.loan.index') }}?status={{ Status::LOAN_PENDING }}&search={{ $user->username }}"
                        overlay_icon=0 icon_style=solid />
                </div>
                <div class="col-xxl-3 col-xl-4 col-sm-6">
                    <x-widget-2 color="success" icon="las la-check-circle" title="Paid Loan"
                        value="{{ $widget['paid_loan'] }}"
                        link="{{ route('admin.loan.index') }}?status={{ Status::LOAN_PAID }}&search={{ $user->username }}"
                        overlay_icon=0 icon_style=solid />
                </div>
                <div class="col-xxl-3 col-xl-4 col-sm-6">
                    <x-widget-2 color="danger" icon="las la-ban" title="Rejected Loan"
                        value="{{ $widget['rejected_loan'] }}"
                        link="{{ route('admin.loan.index') }}?status={{ Status::LOAN_REJECTED }}&search={{ $user->username }}"
                        overlay_icon=0 icon_style=solid />
                </div>
            </div>
        </div>
                    ...
                    <div class="card mb-4">
                        <div class="card-header">
                            <div class="card-title d-flex justify-content-center gap-3">
                                <h6>
                                    <i
                                        class="la @if ($user->ev) la-check-circle text--success @else la-times-circle text--danger @endif"></i>
                                    @lang('Email')
                                </h6>
                                <h6>
                                    <i
                                        class="la  @if ($user->sv) la-check-circle text--success @else la-times-circle text--danger @endif"></i>
                                    @lang('Mobile')
                                </h6>
                                <h6>
                                    <i
                                        class="la @if ($user->kv) la-check-circle text--success @else la-times-circle text--danger @endif"></i>
                                    @lang('KYC')
                                </h6>
                            </div>
                        </div>
                        <div class="card-body text-center">
                            <img class="account-holder-image rounded border w-100"
